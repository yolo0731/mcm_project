...
def create_interpretability_report(all_results, evaluation_results):
    ...
    with open(report_path, 'w', encoding='utf-8') as f:
        ...
        f.write("4.3 物理机理验证\n")
        physical_val = all_results['post_analysis']['physical_validation']
        f.write(f"- 故障频率一致性验证: {'通过' if physical_val['frequency_validation']['is_valid'] else '未通过'}\n")
        f.write(f"- 包络解调验证: {'通过' if physical_val['envelope_validation']['is_valid'] else '未通过'}\n")
        f.write(f"- 整体有效性: {'有效' if physical_val['overall_validity'] else '无效'}\n\n")
...
def generate_sample_signal(length=2048, sampling_rate=12000):
    """生成示例信号"""
    np.random.seed(42)
    t = np.arange(length) / sampling_rate

    # 生成包含故障频率的信号
    signal = 0.5 * np.sin(2 * np.pi * 30 * t)  # 转频
    signal += 0.3 * np.sin(2 * np.pi * 162.2 * t)  # BPFI
    signal += 0.2 * np.sin(2 * np.pi * 324.4 * t)  # 2×BPFI
    signal += 0.1 * np.random.randn(length)  # 噪声
